...
- `TELEGRAM_BOT_TOKEN`
- `TELEGRAM_ADMIN_IDS` (CSV tg_id админов)
...
- API:
  ...
  - `reserveItems(items:OrderItem[], order_id?:number): Promise<void>` — транзакционно: проверка остатков (Sheets.qty_available - qty_reserved >= qty), запись в `reservations` с TTL, увеличение `qty_reserved`.
